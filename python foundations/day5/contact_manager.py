...
class ContactManager:
    def __init__(self,PATH=Path("storage.csv")):

            self.contacts:list[Contact.Contact]=storage.import_contacts(PATH)
        
        
    def search_contact_name(self,Name:str="")->Contact.Contact|None:
    ...
    def display_contacts(self)->list | None:
        if self.contacts:
            display_list=[]
    ...
    def save_contacts(self,PATH=Path("storage.csv"))->None:
        storage.export_contacts(self.contacts,PATH)